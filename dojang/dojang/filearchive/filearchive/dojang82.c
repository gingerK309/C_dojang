...
int main(int argc, char* argv[]) {
	...
	if(strcmp(command,"append")==0) {//파일 추가 명령
		if (!isExist(archive, filename)) {
			//아카이브 파일에 추가할 파일이 있는지 검사
			if (append(archive, filename) == -1) printf("%s 파일 추가 실패\n",filename);
	        
		}
		else printf("이미 %s 파일이 추가되어 있습니다.\n", filename);
	}
	else if (strcmp(command, "list") == 0) list(archive); //파일 목록 출력 명령
	else if (strcmp(command, "extract") == 0) { //파일 추출 명령
		if (isExist(archive, filename)) { //아카이브 파일에 추출할 파일이 있는지 검사
			if (extract(archive, filename) == -1) printf("%s 파일 추출 실패\n", filename);
		}
		else printf("%s 파일이 없습니다.", filename);
	}
